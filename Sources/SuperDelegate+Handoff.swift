...
public protocol HandoffCapable: UserActivityCapable {
    ...
    func didFailToContinue(userActivityType: String, error: NSError)
}
...
extension SuperDelegate {
    ...
    final public func application(_ application: UIApplication, didUpdate userActivity: NSUserActivity) {
        guard let userActivityCapableSelf = self as? HandoffCapable else {
            noteImproperAPIUsage(text: "Received didUpdateUserActivity but \(self) does not conform to HandoffCapable. Not handling handoff event.")
            return
        }
    ...
    final public func application(_ application: UIApplication, didFailToContinueUserActivityWithType userActivityType: String, error: NSError) {
        guard let userActivityCapableSelf = self as? HandoffCapable else {
            noteImproperAPIUsage(text: "Received didFailToContinueUserActivityWithType but \(self) does not conform to HandoffCapable. Not handling handoff event.")
            return
        }
