...
public protocol ShortcutCapable: ApplicationLaunched {
    ...
    func handle(shortcutItem: UIApplicationShortcutItem, completionHandler: () -> Void)
}
...
extension SuperDelegate {
    ...
    @available(iOS 9.0, *)
    final public func application(_ application: UIApplication, performActionFor shortcutItem: UIApplicationShortcutItem, completionHandler: (Bool) -> Void) {
        guard let shortcutCapableSelf = self as? ShortcutCapable else {
            noteImproperAPIUsage(text: "Received shortcut item but \(self) does not conform to ShortcutCapable. Ignoring.")
