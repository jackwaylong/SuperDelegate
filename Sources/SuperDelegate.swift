...
public class SuperDelegate: NSObject, UIApplicationDelegate {
    ...
    public func setup(mainWindow: UIWindow) {
        guard withinLoadInterface else {
            noteImproperAPIUsage(text: "Must call \(#function) from within loadInterface(launchItem:)")
            return
        }
    ...
    final public func application(_ application: UIApplication, willFinishLaunchingWithOptions launchOptions: [NSObject : AnyObject]?) -> Bool {
        ...
        case let .shortcut(launchShortcutItem):
            if #available(iOS 9.0, *) {
                guard let shortcutCapableSelf = self as? ShortcutCapable else {
                    noteImproperAPIUsage(text: "Received shortcut item but \(self) does not conform to ShortcutCapable. Not handling shortcut.")
                    return true
                }
                ...
                noteImproperAPIUsage(text: "Launched due to ShortcutItem but not running iOS 9 or later.")
                launchItem = .none
            }
            
        case let .userActivity(launchUserActivity):
            guard let userActivityCapableSelf = self as? UserActivityCapable else {
                noteImproperAPIUsage(text: "Received user activity item but \(self) does not conform to UserActivityCapable. Not handling user activity.")
                return false
            }
        ...
        case let .openURL(launchURLToOpen):
            guard let openURLCapableSelf = self as? OpenURLCapable else {
                noteImproperAPIUsage(text: "Received openURL action but \(self) does not conform to OpenURLCapable. Not handling URL.")
                return false
            }
    ...
    final public func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject : AnyObject]?) -> Bool {
        ...
        if let launchBluetoothPeripheralIdentifiers = launchOptions?[UIApplicationLaunchOptionsBluetoothPeripheralsKey] as? [String] {
            guard let backgroundBluetoothPeripheralCapableSelf = self as? BackgroundBluetoothPeripheralCapable else {
                noteImproperAPIUsage(text: "Received background bluetooth peripheral restore identifier but \(self) does not conform to BackgroundBluetoothPeripheralCapable. Failing to launch app.")
                return false
            }
            
            backgroundBluetoothPeripheralCapableSelf.restoreBluetoothPeripheralManagers(identifiersToRestore: launchBluetoothPeripheralIdentifiers)
        }
        ...
        if let launchBluetoothCentralIdentifiers = launchOptions?[UIApplicationLaunchOptionsBluetoothCentralsKey] as? [String] {
            guard let backgroundBluetoothCentralCapableSelf = self as? BackgroundBluetoothCentralCapable else {
                noteImproperAPIUsage(text: "Received background bluetooth peripheral restore identifier but \(self) does not conform to BackgroundBluetoothCentralCapable. Failing to launch app.")
                return false
            }
        ...
        DispatchQueue.main.after(when: DispatchTime.now() + DispatchTimeInterval.seconds(5), execute: {
